...
**Creative North Star: 报告纸上的仪器面板**

LabPlot Lite 的界面应像一个可检查的实验绘图工具，而不是营销网站、AI 工具或复杂科研软件。默认视觉是浅色报告纸、冷静灰阶、清晰表格和白底图表。功能模式允许更高密度，但控件必须稳定、可预测。

这套方向综合四类参考：Carbon 的数据密集产品结构，Spectrum 的专业工具控件密度，GOV.UK 的低学习成本表单规范，Datawrapper / Prism 的图表生成路径。功能模式遵循 Datawrapper 的四步结构；最后一步在 LabPlot Lite 中映射为本地检查和下载文件。借鉴的是原则，不是复制品牌。

实现层面以 Carbon + Plotly 为准：Carbon Web Components 接管通用工作台 UI，Plotly.js 接管科研绘图和图表导出。LabPlot Lite 的自有界面代码应尽量薄，只负责产品流程、数据状态和必要布局。详细工程边界集中维护在 [`docs/engineering/frontend-contract.md`](docs/engineering/frontend-contract.md)，本文件只保留设计规则。

官方依据集中记录在 [`docs/design/carbon-resources.md`](docs/design/carbon-resources.md)。新增或重构通用 UI 时先查 Carbon 组件和 Web Components 指南；调整坐标轴、图例、网格、颜色、标签或图表说明时先查 IBM / Carbon data visualization 指南。LabPlot Lite 借用 Carbon 和 IBM Design Language 的系统原则，不借 IBM logo、Brand Center、摄影、商标或 co-branding 身份。

**Key Characteristics:**

- 浅色优先：页面背景接近实验报告纸，主内容用白色或浅灰面板承载。
- 图表可检查：结果画布保持白底、细网格、清晰轴线和可读图例。
...
**The Short Help Rule.** 文案只说明动作、状态、判断依据或风险。不要重复标题，不要写泛化形容词。

## 4. Layout

首页是产品 landing，但第一屏必须直接展示图表输出和流程。工作台是工具，不是展示页。
...
代码边界、owner 文件、依赖规则和验证命令以 [`docs/engineering/frontend-contract.md`](docs/engineering/frontend-contract.md) 为准。设计层只保留原则：Carbon 管通用 UI，Plotly 管图表，项目 CSS 只做布局、间距、宽度、品牌 token 和必要适配。
...
- Don't 使用深色渐变、玻璃拟态、彩色 glow 或模板化工具界面。
