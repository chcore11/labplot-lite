...
1. 先读 `AGENTS.md` 和 `docs/INDEX.md`。
2. 根据任务阅读 `PRODUCT.md`、`DESIGN.md` 或 `docs/engineering/frontend-contract.md`。
3. 较大改动先按 `docs/workflows/project-change-workflow.md` 列计划、实施、自审和验证。
4. UI、设计、布局或交互改动先运行：
...
5. 不要引入与 Carbon 并行的通用 UI 系统。
6. 不要引入与 Plotly 并行的科研绘图引擎。
7. 新增通用 UI 前先查 Carbon Web Components 和项目现有 `cds-*` 用法；Carbon 能覆盖时只写状态 glue，不自写控件。
