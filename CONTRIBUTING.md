...
3. 不要引入与 Carbon 并行的通用 UI 系统。
4. 不要引入与 Plotly 并行的科研绘图引擎。
5. 新增通用 UI 前先查 Carbon Web Components 和项目现有 `cds-*` 用法；Carbon 能覆盖时只写状态 glue，不自写控件。
...
涉及 Plotly、拟合、下载或素材包时，至少验证一次完整流程：
