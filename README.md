...
- `temperature_time.csv`：简易模式温度-时间曲线。
- `voltage_current.csv`：简易模式电压-电流线性关系。
- `concentration_absorbance.csv`：简易模式浓度-吸光度标准曲线。
- `cooling_curve.csv`：简易模式冷却曲线。
- `free_fall_fit.csv`：简易模式自由落体拟合数据。
